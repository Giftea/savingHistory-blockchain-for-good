    ...
    StakingStorageV1
{
     ...
     * @param _SSAVEH                Address of the SSAVEH Token
     * @param _donationMiner        Address of the DonationMiner
     * @param _cooldown             Number of blocks after a user can claim an unstake
     */
    function initialize(
        IERC20 _SAVEH,
        IMintableERC20 _SSAVEH,
        IDonationMiner _donationMiner,
        uint256 _cooldown
    ) public initializer {
        require(address(_SAVEH) != address(0), "Stake::initialize: invalid _SAVEH address");
        require(address(_SSAVEH) != address(0), "Stake::initialize: invalid _SSAVEH address");
        require(
        ...
        )
    {
        ...
        return (
            ...
            SSAVEH.balanceOf(_holderAddress) - _holder.amount
        );
    }
     ...
     * @param _amount Amount of usdc tokens to stake
     */
    function stake(address _holderAddress, uint256 _amount) external override nonReentrant {
        ...
        IERC20Upgradeable(address(SAVEH)).safeTransferFrom(msg.sender, address(this), _amount);
        SSAVEH.mint(_holderAddress, uint96(_amount));
    ...
    function _claim(uint256 _lastUnstakeId) internal returns (uint256) {
        ...
        SSAVEH.burn(msg.sender, uint96(_amount));
        IERC20Upgradeable(address(SAVEH)).safeTransfer(msg.sender, _amount);
